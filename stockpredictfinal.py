...
main = tkinter.Tk()
main.title("Stock Market Using KNN")
...
def rfregressor():
	...
	regressorrf = regressor.score(X_train, y_train)	
	text.insert(END, "Accuracy of RFREGRESSOR with Uniform weights : "+str(regressorrf*100)+"\n\n")
...
font1 = ('times', 14, 'bold')
uploadButton = Button(main, text="Load Dataset", command=loadDataset)
...
distButton = Button(main, text="Run RandomForestRegressor ", command=rfregressor)
distButton.place(x=700,y=350)
distButton.config(font=font1)


predButton = Button(main, text="Predict the Test Data ", command=predModel)
predButton.place(x=700,y=400)
